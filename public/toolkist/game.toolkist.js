export var game = (function($) {
    ...
    game.Playlist = class
    {
        ...
        AddLevel(uid, workshopId, name, author)
        {
            this.levels.push({UID : uid, WorkshopID : workshopId, Name : name, Author : author});
        ...
        SetProperties(properties)
        {
            ...
            this.roundLength = properties.roundLength;
            this.shufflePlaylist = properties.shuffle;
        }

        Merge(playlist)
        {
            this.levels = this.levels.concat(playlist.levels);
        }

        RemoveLevel(index)
        {
            if(this.levels.length == 0 || index < 0 || index >= this.levels.length)
            {
                return;
            }

            this.levels = this.levels.splice(index, 1);
        }
            ...
            try {
                ...
                if (
                    ...
                    for (const levelData of jsonData.levels) {
                        this.AddLevel(levelData.UID, levelData.WorkshopID, levelData.Name, levelData.Author);
                    }
                } else {
    ...
    game.Zeeplevel = class
    {
        ...
        Generate12DigitRandomNumber(){
            return String(Math.floor(Math.random() * 9) + 1) + String(Math.floor(Math.random() * 10 ** 11)).padStart(11, '0');
        }
    ...
    game.painting = {
        ...
        {
            var id = this.paints.hasOwnProperty(id) ? id : 6;
