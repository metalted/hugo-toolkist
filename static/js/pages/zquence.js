...
export function init()
{
    ...
    $('#pagepanel1').append(controls.appContainer);
    sequencer = new zquence.Sequencer();
}

function constructPage()
{
    let ctrl = {
        appContainer: $('<div>').attr({ 'id': 'app-container' }),
    
        leftColumnContainer: $('<div>').attr({ 'id': 'left-column-container' }),
        tracklistContainer: $('<div>').attr({ 'id': 'track-list-container' }),
        tracklistToolbar: $('<div>').attr({ 'id': 'track-list-toolbar' }),
    
        addPianoButton: $('<div>')
            .attr({ 'id': 'add-piano-button' })
            .addClass('add-instrument-button')
            .append(
                $('<img>').attr({ 'src': '/img/piano.png' }),
                $('<span>').text('+')
            ),
        addTrumpetButton: $('<div>')
            .attr({ 'id': 'add-trumpet-button' })
            .addClass('add-instrument-button')
            .append(
                $('<img>').attr({ 'src': '/img/trumpet.png' }),
                $('<span>').text('+')
            ),
        addFluteButton: $('<div>')
            .attr({ 'id': 'add-flute-button' })
            .addClass('add-instrument-button')
            .append(
                $('<img>').attr({ 'src': '/img/flute.png' }),
                $('<span>').text('+')
            ),
        addKazooButton: $('<div>')
            .attr({ 'id': 'add-kazoo-button' })
            .addClass('add-instrument-button')
            .append(
                $('<img>').attr({ 'src': '/img/kazoo.png' }),
                $('<span>').text('+')
            ),
        addBlarghlButton: $('<div>')
            .attr({ 'id': 'add-blarghl-button' })
            .addClass('add-instrument-button')
            .append(
                $('<img>').attr({ 'src': '/img/blarghl.png' }),
                $('<span>').text('+')
            ),
    
        tracklist: $('<div>').attr({ 'id': 'track-list' }),
    
        exportContainer: $('<div>').attr({ 'id': 'export-container' }),
        saveButton: $('<div>')
            .attr({ 'id': 'save-button' })
            .addClass('control-button')
            .text('Save Zquence JSON'),
        loadButton: $('<div>')
            .attr({ 'id': 'load-button' })
            .addClass('control-button')
            .text('Load Zquence JSON'),
        loadInput: $('<input>').attr({
            'type': 'file',
            'id': 'load-file',
            'style': 'display:none'
        }),
        importMidiButton: $('<div>')
            .attr({ 'id': 'import-midi-button' })
            .addClass('control-button')
            .text('Import MIDI'),
        importMidiInput: $('<input>').attr({
            'type': 'file',
            'id': 'load-midi',
            'style': 'display:none'
        }),
    
        targetSpeedInput: $('<input>').attr({
            'id': 'targetspeed-input',
            'type': 'number',
            'min': '20',
            'value': '60',
            'placeholder': 'TargetSpeed'
        }),
        exportTypeSelect: $('<select>').attr({ 'id': 'exportType' })
            .append(
                $('<option>').attr({ 'value': 'circle' }).text('Circle'),
                $('<option>').attr({ 'value': 'line' }).text('Line'),
                $('<option>').attr({ 'value': 'road' }).text('Road')
            ),
        exportNameInput: $('<input>').attr({
            'type': 'text',
            'id': 'exportName',
            'value': 'ToolkistZquence'
        }),
        exportButton: $('<div>')
            .attr({ 'id': 'export-button' })
            .addClass('control-button')
            .text('Export to Zeeplevel'),
    
        timeLineContainer: $('<div>').attr({ 'id': 'time-line-container' }),
        timeLineToolbar: $('<div>').attr({ 'id': 'time-line-toolbar' }),
        playButton: $('<div>')
            .attr({ 'id': 'play-button' })
            .addClass('time-line-control-button')
            .append($('<img>').attr({ 'src': '/img/play.png' })),
        pauseButton: $('<div>')
            .attr({ 'id': 'pause-button' })
            .addClass('time-line-control-button')
            .append($('<img>').attr({ 'src': '/img/stop.png' })),
    
        bpmLabel: $('<label>').attr({ 'for': 'bpm-input' }).text(' BPM: '),
        bpmInput: $('<input>').attr({
            'id': 'bpm-input',
            'type': 'number',
            'min': '30',
        ...
        notesPerBarLabel: $('<label>').attr({ 'for': 'notesPerBarSelection' }).text(' Notes/Bar: '),
        notesPerBarSelect: $('<select>').attr({ 'id': 'notesPerBarSelection' })
        .append(
            $('<option>').attr({ 'value': '4' }).text('4'),
            $('<option>').attr({ 'value': '8' }).text('8'),
            $('<option>').attr({ 'value': '16' }).text('16'),
            $('<option>').attr({ 'value': '32' }).text('32')
        ),

        removeBarButton: $('<div>')
            .attr({ 'id': 'remove-bar-button' })
            .addClass('time-line-menu-button')
            .text('Bar -'),
        addBarButton: $('<div>')
            .attr({ 'id': 'add-bar-button' })
            .addClass('time-line-menu-button')
            .text('Bar +'),

        sequencer: $('<div>').attr({ 'id': 'sequencer' })
    };

    ctrl.tracklistToolbar.append(
        ctrl.addPianoButton,
        ctrl.addTrumpetButton,
        ctrl.addFluteButton,
        ctrl.addKazooButton,
        ctrl.addBlarghlButton
    );

    ctrl.tracklistContainer.append(
        ctrl.tracklistToolbar,
        ctrl.tracklist
    );

    ctrl.exportContainer.append(
        ctrl.saveButton,
        ctrl.loadButton,
        ctrl.loadInput,
        ctrl.importMidiButton,
        ctrl.importMidiInput,
        $('<table>').append(
            $('<tr>').append($('<th>').text('Target Speed:'), $('<td>').append(ctrl.targetSpeedInput)),
            $('<tr>').append($('<th>').text('Export Type:'), $('<td>').append(ctrl.exportTypeSelect)),
            $('<tr>').append($('<th>').text('Filename:'), $('<td>').append(ctrl.exportNameInput))
        ),
        ctrl.exportButton
    );

    ctrl.timeLineToolbar.append(
        ctrl.playButton,
        ctrl.pauseButton,
        ctrl.bpmLabel,
        ctrl.bpmInput,
        ctrl.notesPerBarLabel,
        ctrl.notesPerBarSelect,
        ctrl.removeBarButton,
        ctrl.addBarButton
    );

    ctrl.timeLineContainer.append(
        ctrl.timeLineToolbar,
        ctrl.sequencer
    );

    ctrl.leftColumnContainer.append(
        ctrl.tracklistContainer,
        ctrl.exportContainer
    );

    ctrl.appContainer.append(
        ctrl.leftColumnContainer,
        ctrl.timeLineContainer
    );

    return ctrl;
}

var zquence = (function($) {
    var zquence = {};
